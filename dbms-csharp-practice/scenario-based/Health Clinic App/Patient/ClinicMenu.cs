...
namespace HealthClinicApp
{
    public class ClinicMenu : IClinicOperations
    {
        ...
        {
            Console.Clear();
            Utility.DisplayHeader("View Patient Visit History");
            ...
            string query = "SELECT a.appointment_date, a.appointment_time, d.name AS doctor_name, v.diagnosis, v.notes FROM visits v JOIN appointments a ON v.appointment_id = a.appointment_id JOIN doctors d ON v.doctor_id = d.doctor_id WHERE v.patient_id = @patientId ORDER BY v.visit_date DESC";
            SqlCommand cmd = new SqlCommand(query, conn);
            ...
            while (reader.Read())
            {
                ...
                Console.WriteLine("Time: " + reader.GetTimeSpan(1).ToString());
                Console.WriteLine("Doctor: " + reader.GetString(2));
                Console.WriteLine("Diagnosis: " + reader.GetString(3));
                Console.WriteLine("Notes: " + reader.GetString(4));
                Console.WriteLine("-----------------------------------");
            }
